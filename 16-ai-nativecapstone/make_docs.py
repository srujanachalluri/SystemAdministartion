...
def screenshot(doc, paths, width_in=6.9):
    """Insert one or more screenshots as the evidence for a step.

    Falls back to the red placeholder for any step whose screenshot is missing,
    so an incomplete run is visible rather than silently blank.
    """
    ...
    for rel in paths:
        ...
        cr = cap.add_run(f"Screenshot: {rel}")
        cr.font.size = Pt(7.5)
        cr.italic = True
...
# ================================================================= REPORT ===
STUDENT_NAME = "Sareena Challuri"
REPO_URL = "https://github.com/srujanachalluri/SystemAdministartion"

# Terminal screenshots captured in GitHub Codespaces, keyed by step number.
# A step with no screenshot falls back to a red placeholder so a gap is visible
# rather than silently blank.
EVIDENCE = {
    "1":  ["results/step-01-environment.png"],
    "2":  ["results/step-02-structure.png"],
    "3":  ["results/step-03a-install.png",
           "results/step-03b-autonomy-ladder.png",
           "results/step-03c-inference-platform.png"],
    "4":  ["results/step-04-rungs-and-owners.png"],
    "5":  ["results/step-05-cost-model.png"],
    "6":  ["results/step-06a-output-head.png",
           "results/step-06b-output-wc.png"],
    "7":  ["results/step-07-owner-coverage.png"],
    "8":  ["results/step-08-generate-docx.png"],
    "9":  ["results/step-09-commit.png"],
    "10": ["results/step-10-push.png"],
}
...
COMMANDS = [
    ...
    ("2", "Show the project structure",
     "cd /workspaces/SystemAdministartion/16-ai-native-capstone\n"
     "ls -R\n"
     "find . -type f | sort",
...
def build_report(path="REPORT.docx"):
    ...
    para(doc, f"Student: {STUDENT_NAME}     Date: ______________")
    para(doc, f"Repository: {REPO_URL}  (folder: 16-ai-native-capstone)")
    rule(doc)
    ...
    for num, title, cmd, why in COMMANDS:
        ...
        screenshot(doc, EVIDENCE.get(num, []))
